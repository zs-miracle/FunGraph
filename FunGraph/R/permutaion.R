#' @title Permutaion 
#' @description By shuffling the relationship between genotype and phenotype repeatedly, 
...
#' @param permutaion_times Number of permutation tests.
...
#' @return LR of each SNP calculated by FunMap 1000 times
#' @export
permutaion<- function(funmap_data,funmap_data_snp,times,permutaion_times,rownumber=dim(funmap_data_snp)[1]){
  ...
  permutaion_threshold<-matrix(NA,rownumber,permutaion_times)
  for(i in 1:permutaion_times){
    resample<- sample(1:dim(funmap_data)[1],dim(funmap_data)[1],replace = F)
    funmap_data <- funmap_data[resample,]
    rownames(funmap_data) <-c(1:dim(funmap_data)[1])
    permutaion_result<- Funmap_function(funmap_data,funmap_data_snp,times,rownumber = rownumber)
    permutaion_threshold[,i]<-permutaion_result$LR
  }
  return(permutaion_threshold)
}
